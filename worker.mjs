...
import { createCodingEngine } from "./src/coding-engine.mjs";
...
const planner=createArchitecturePlanner();
const sandbox=createSandbox();
const codingEngine=createCodingEngine({agent,planner,commandPolicy:createCommandPolicy({allowDependencies:process.env.SILO_ALLOW_DEPENDENCIES==="true",allowNetwork:process.env.SILO_ALLOW_NETWORK==="true"})});
...
async function work(job){if(job.type==="agent")return agent.generate(job.payload);if(job.type==="sandbox")return sandbox.run(job.payload);if(job.type==="build")return codingEngine.build(job.payload);throw new Error(`Unsupported job type ${job.type}`);}
